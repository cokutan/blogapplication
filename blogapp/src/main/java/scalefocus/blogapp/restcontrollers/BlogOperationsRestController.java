...
public class BlogOperationsRestController {
    ...
    public ResponseEntity<List<Blog>> getSummaryListForUser(
            @Parameter(description = "name of the user to retrieve blogs") @PathVariable String username, @Parameter(description = "which page to retrieve") @RequestParam(value = "page", required = false) Integer page, @Parameter(description = "how many records to retrieve") @RequestParam(value = "size", required = false) Integer size) {
        List<Blog> blogs = blogService.getBlogSummaryListForUser(username, Optional.ofNullable(page).orElseGet(() -> 0), Optional.ofNullable(size).orElseGet(() -> 100));
